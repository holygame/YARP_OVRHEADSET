...
void yarp::dev::OVRHeadset::run()
{
    ...
    if (ts.StatusFlags & ovrStatus_OrientationTracked) 
	{
		...
		{
			if (initialOrientationInitialised = false)
			{
				/*using namespace std::literals::chrono_literals;
				std::this_thread::sleep_for(3s);*/

				initialOrientation = headpose.ThePose.Orientation;
				initialOrientationInitialised = true;
			}
			OVR::Quatf orientation(headpose.ThePose.Orientation.x - initialOrientation.x,
					headpose.ThePose.Orientation.y - initialOrientation.y,
					predicted_headpose.ThePose.Orientation.z - initialOrientation.z,
					predicted_headpose.ThePose.Orientation.w - initialOrientation.w);
			float yaw, pitch, roll;
			...
			output_orientation.addDouble(pitch);
			output_orientation.addDouble(-roll);
			output_orientation.addDouble(yaw);
			orientationPort->setEnvelope(stamp);
			orientationPort->write();
        }

        writeVec3OnPort(angularVelocityPort,     headpose.AngularVelocity,     stamp);
        writeVec3OnPort(angularAccelerationPort, headpose.AngularAcceleration, stamp);

    } else 
	{
        // Do not warn more than once every 5 seconds
		initialOrientationInitialised = false;
    ...
    if (predicted_ts.StatusFlags & ovrStatus_OrientationTracked) {
        ...
        if (predictedOrientationPort->getOutputCount() > 0) {
			...
			OVR::Quatf orientation(predicted_headpose.ThePose.Orientation.x - initialOrientation.x,
				predicted_headpose.ThePose.Orientation.y - initialOrientation.y,
				predicted_headpose.ThePose.Orientation.z - initialOrientation.z,
				predicted_headpose.ThePose.Orientation.w - initialOrientation.w);
            float yaw, pitch, roll;
            orientation.GetEulerAngles<OVR::Axis_Y, OVR::Axis_X, OVR::Axis_Z>(&yaw, &pitch, &roll);
        ...
        writeVec3OnPort(predictedAngularVelocityPort,     predicted_headpose.AngularVelocity,     stamp);
        writeVec3OnPort(predictedAngularAccelerationPort, predicted_headpose.AngularAcceleration, stamp);
